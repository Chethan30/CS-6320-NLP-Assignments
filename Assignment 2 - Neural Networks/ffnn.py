...
class FFNN(nn.Module):
    ...
    def forward(self, input_vector):
        ...
        # Try adding hidden layers using the loop
        a = input_vector
        z = self.W1(a)
        while self.h > 5:
            a = self.activation(z)
            h_new = self.h - 2
            z = nn.Linear(self.h, h_new)(a)
            self.h = h_new
        predicted_vector = self.softmax(z)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        minibatch_size = 16 
        N = len(train_data) 
        print("length of training data: {}".format(N))
        ...
        minibatch_size = 16 
        N = len(valid_data) 
        print("length of validation data: {}".format(N))
        for minibatch_index in tqdm(range(N // minibatch_size)):
            optimizer.zero_grad()
            loss = None
            for example_index in range(minibatch_size):
        ...
        minibatch_size = 16 
        N = len(test_data) 
        print("length of test data: {}".format(N))
        for minibatch_index in tqdm(range(N // minibatch_size)):
            optimizer.zero_grad()
            loss = None
            for example_index in range(minibatch_size):
        ...
        output_values.append(((epoch+1), training_accuracy, validation_accuracy))
